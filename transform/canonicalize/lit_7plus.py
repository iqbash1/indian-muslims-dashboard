...
def _cells(l2_path: pathlib.Path) -> dict[tuple[str, str, str, str, str], int]:
    """(level, code, religion, measure, age_group) -> value, residence=total, persons."""
    ...
    with l2_path.open() as f:
        for row in csv.DictReader(f):
            ...
            cells[(level, code, row["religion"], row["measure"], row["age_group"])] = int(row["value"])
    return cells
...
def canonicalize() -> None:
    ...
    for year, cells, (src_id, src_doc, note) in rounds:
        geographies = {(lv, cd) for (lv, cd, _, _, _) in cells}
        for (level, code) in geographies:
            for religion in OUTPUT_RELIGIONS:
                ...
                total_nostate = cells.get((level, code, religion, "total_population", "Age not stated"), 0)
                literate_all = cells.get((level, code, religion, "literate", "Total"))
                literate_nostate = cells.get((level, code, religion, "literate", "Age not stated"), 0)
                ...
                out_rows.append([
                    "lit-7plus", level, code, year, religion,
                    rate, DENOMINATOR, "", "", "",
